...
tags: [Bypass, CTF, Nmap, Curl, Python Debugger, SSRF]
---
...
![Owned Date](/assets/images/hackthebox/forge/owned_date.png)
...
Tenemos dos posibles métodos para subir un archivo, archivo local en nuestra máquina o mediante una URL.
...
En principio no tenemos limitaciones en cuanto a formato (aunque arriba a la derecha indique que subamos una imagen), una vez subido nuestro archivo nos genera un enlace.
...
Al tratar de abrir el enlace generado nos indica que no es posible mostrar la imagen porque contiene errores.
...
Vemos que han dejado escritas en texto plano las credenciales para conectarse a un servidor FTP interno.

> An internal ftp server has been setup with credentials as **user:heightofsecurity123!**

Ahora podemos construir el siguiente enlace, subirlo y repetir el proceso con curl para ver el resultado:
...
Una vez tengamos la información del archivo, lo creamos en nuestra máquina y ya podemos conectarnos por SSH.
...
Necesitamos escapar de este menú, algo bastante fácil si hemos entendido el código Python. Fijaos en esta línea:

```python
option = int(clientsock.recv(1024).strip())
```

Está convirtiendo a entero el valor que se introduce en el script como opción (el menú), pero no está verificando que el input sea explícitamente un número,
...
#### Una máquina entretenida de realizar, a pesar de considerarse de dificultad media me ha resultado bastante fácil todo el proceso de intrusión. Útil para reforzar conocimientos ya adquiridos previamente. Nos vemos en un próximo. ¡Feliz hacking! :skull: